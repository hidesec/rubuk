...
export default function PopularBooks() {
  ...
  useEffect(() => {
    fetch("/api/books").then((r) => r.json()).then((d) => { if (d.ok) setBooks(d.books.slice(0, 6)); });
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12">
          <div className="space-y-3">
            <p className="text-xs uppercase tracking-[0.2em] text-gold font-semibold">Koleksi Kami</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-espresso tracking-tight">Buku yang Sedang Dibahas</h2>
            <p className="text-warm-gray max-w-lg leading-relaxed">Daftar buku yang menjadi bahan diskusi utama di komunitas kami.</p>
          </div>
          <Link href="/books" className="inline-flex items-center gap-2 text-sm font-medium text-mocha hover:text-espresso transition-colors">
            Lihat Semua
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {books.map((book) => (
            <div key={book.id} className="group p-5 rounded-2xl bg-parchment border border-cream-dark/30 hover:border-cream-dark/60 hover:shadow-lg hover:shadow-espresso/5 transition-all duration-300">
